	...
	if (!SessionInterface->FindSessions(*LocalPlayer->GetPreferredUniqueNetId(), LastSessionSearch.ToSharedRef())) {
		...
		if (GEngine) {
			GEngine->AddOnScreenDebugMessage(
				...
				15.f,
				FColor::Yellow,
				FString(TEXT("Try FindSessions Fail"))
			);
		}
...
void UMultiplayerSessionsSubsystem::OnFindSessionsComplete(bool bWasSuccessful) {
	...
	if (GEngine) {
		GEngine->AddOnScreenDebugMessage(
			...
			15.f,
			FColor::Yellow,
			FString(TEXT("OnFindSessionsComplete back"))
		);
	}
	...
	if (LastSessionSearch->SearchResults.Num() <= 0) {
		if (GEngine) {
			GEngine->AddOnScreenDebugMessage(
				...
				15.f,
				FColor::Yellow,
				FString(TEXT("OnFindSessionsComplete back can not find"))
			);
		}
	...
	if (GEngine) {
		GEngine->AddOnScreenDebugMessage(
			...
			15.f,
			FColor::Yellow,
			FString(TEXT("OnFindSessionsComplete success"))
		);
	}
